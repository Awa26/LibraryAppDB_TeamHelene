...
public class us05_stepDefs_BA {
    LoginPage loginPage = new LoginPage();
    BookPage bookPage = new BookPage();
    @Given("Establish the database connection BA")
    public void establish_the_database_connection_ba() {
        System.out.println("hooks runs this");
    }
    @When("I execute query to find most popular book genre BA")
    public void i_execute_query_to_find_most_popular_book_genre_ba() {
        DB_Util.runQuery("select bc.name,count(*) from book_borrow bb\n" +
                "inner join books b on bb.book_id = b.id\n" +
                "inner join book_categories bc on b.book_category_id=bc.id\n" +
                "group by name order by 2 desc");
    }
    @Then("verify {string} is the most popular book genre BA")
    public void verify_is_the_most_popular_book_genre_ba(String Fantasy) {
